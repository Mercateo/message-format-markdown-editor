...
function extractHintsFromAst(astNode) {
  switch(astNode.type) {
    ...
    case "invalidEscapeChar":
      return [];// overriding all escape sequences so that markdown escapes aren't considered errors [createAnnotation("Invalid escape character: " + astNode.text, "error", astNode.start, astNode.end)];
    case "invalidLiteral":
